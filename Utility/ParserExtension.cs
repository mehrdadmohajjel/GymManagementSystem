...
namespace System
{
    public static class ParserExtension
    {
        ...
        public static DateTime ToDateTime(this string persianDateString)
        {
            var pc = new PersianCalendar();

            string datePart = persianDateString;
            var timeValues = new int[2];

            if (persianDateString.Contains(":"))
            {
                datePart = persianDateString.Split(' ')[0];
                timeValues = persianDateString.Split(' ')[1].Split(':').Select(o => int.Parse(o)).ToArray();
            }

            var dateValues = datePart.Split(new[] { "/" }, StringSplitOptions.RemoveEmptyEntries)
                .Select(o => int.Parse(o)).ToArray();

            return pc.ToDateTime(dateValues[0], dateValues[1], dateValues[2], timeValues[0], timeValues[1], 0, 0);
        }
        #endregion
